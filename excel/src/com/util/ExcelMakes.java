...
public class ExcelMakes { 
	...
	private  void exportExcel(List<? extends Object> dataList){
		...
		try {
			...
			for (int i = 0; i < cellNamesTop.length; i++) {
					 
							// 设置标题格式
							WritableFont wfTitle = new jxl.write.WritableFont(
							WritableFont.COURIER, titile_font_size, WritableFont.NO_BOLD, false);
							WritableCellFormat wcfTitle = new WritableCellFormat(wfTitle);
							// 设置垂直对齐方式
							wcfTitle.setVerticalAlignment(VerticalAlignment.CENTRE);// 设置对齐方式
							// 设置水平对齐方式
							wcfTitle.setAlignment(jxl.format.Alignment.CENTRE); // 设置对齐方式
						 Label title=new Label(i,rows,cellNamesTop[i],wcfTitle);
					 	 sheetOne.addCell(title); 
					 	 //设置行高
					 	 sheetOne.setRowView(0,350);
					 	// rows=rows+1;
			 ...
			 */
			for (int i = 0; i < cellNamesTop.length; i++) {			 
				if(i>0&&!cellNamesTop[i].trim().equals("")){
				 	 //合并单元格
				 	 sheetOne.mergeCells(i, rows, i+2,rows);
				}
			}
			/****
			 * 添加第二列 列名称
	...
	*/
	...
	public String[] getCellNamesTop() {
		return cellNamesTop;
	}


	public void setCellNamesTop(String[] cellNamesTop) {
		this.cellNamesTop = cellNamesTop;
	}
...
class ExcelService {
	...
	public static int getDATA_COUNT() {
		return DATA_COUNT;
	}

	public static void setDATA_COUNT(int dATA_COUNT) {
